...
/*   Updated: 2025/12/13 19:31:29 by stkabang         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (ray->side == 0)
		ray->perp_dist_wall = (ray->map_x - player->pos_x
				+ (1 - ray->step_x) / 2) / ray->ray_dir_x;
	else
		ray->perp_dist_wall = (ray->map_y - player->pos_y
...
void	wall_bounds(t_ray *ray, int *draw_start, int *draw_end,
				double *line_height)
{
